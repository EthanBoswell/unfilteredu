...
def process_school(slug: str, path: str, schools_by_slug: dict) -> None:
    ...
    has_all_scores = has_all_categories and all("score" in summary[cat] for cat in CATEGORIES)

    if has_all_scores:
        print(f"[SKIP] {slug}: already scored")
        return
    ...
    if not has_all_categories:
        ...
        if not school or not os.path.exists(raw_path):
            print(f"[SKIP] {slug}: missing new categories but no school/raw data to re-summarize from")
            return

        print(f"[RESUMMARIZE] {slug}: missing new categories, re-running full summarization...")
        os.remove(path)
        with_rate_limit_retry(summarize_school, school, raw_path, DATA_DIR)
        return

    print(f"[SCORE] {slug}: scoring existing categories...")
    scores = with_rate_limit_retry(score_summary, summary)
    for cat in CATEGORIES:
        if "score" not in summary[cat]:
            if cat in scores:
